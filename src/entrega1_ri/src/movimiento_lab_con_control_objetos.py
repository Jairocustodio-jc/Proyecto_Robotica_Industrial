...
import time
import numpy as np
from entrega1_ri.msg import ArrayXY  # Suponiendo que el mensaje ArrayXY está definido en este paquete

class PositionController:
    def __init__(self, targets, pos_kp, pos_ki, pos_kd, orient_kp, orient_ki, orient_kd, tolerance=0.01):
        rospy.init_node('position_controller')
        rospy.Subscriber('/odom', Odometry, self.odometry_callback)
        rospy.Subscriber('lidar_xy', ArrayXY, self.lidar_callback)
        ...
        self.object_is_near = False
        self.object_detected_time = None
        self.was_stopped = False  # Nueva bandera para detectar cambios de estado
        self.permanently_stopped = False  # Nueva bandera para detenerse permanentemente
    ...
    def odometry_callback(self, odom):
        ...
        if self.object_is_near:
            if not self.was_stopped:
                rospy.loginfo("Detenido debido a la detección de un objeto cercano.")
                self.was_stopped = True
            # Pausar el movimiento si un objeto está cerca
            ...
            if self.was_stopped:
                rospy.loginfo("Reanudando el movimiento.")
    ...
    def lidar_callback(self, msg):
        ...
        if min_distance < 0.45:
            if self.object_detected_time is None:
                self.object_detected_time = rospy.Time.now().to_sec()
                self.object_is_near = True
            else:
                elapsed_time = rospy.Time.now().to_sec() - self.object_detected_time
                if elapsed_time > 2:
                    self.object_is_near = True
                    self.permanently_stopped = True
                    rospy.loginfo("Objeto cerca demasiado tiempo. Detenido permanentemente.")
        else:
            self.object_detected_time = None
            self.object_is_near = False
